...
## figure directive and fenced payload have been parsed.  It preserves the
## writing-repository compatibility behavior for `select`, `render`, and
## `replace` modes without yet introducing the generalized input/output plugin
## architecture reserved for the next implementation phase.
##
## The module reads processor configuration supplied by the Bash orchestrator via
## `-v` variables and mutates the current figure state maintained by the main AWK
## rule.  File paths are derived only from previously validated figure identifiers.
## The implementation targets portable AWK.

## @fn emit_embedded(path, kind)
## @brief Re-emits a materialized text or DOT asset as a safe Markdown fence.
## @details
...
##
## @param path Path of the materialized text or DOT asset to read.
...
## `render` mode, the payload is materialized beneath `figdir`.  In `replace`
## mode, text and DOT assets are re-embedded while graphical outputs become
## Markdown image references.
##
## The function assumes `id` has already passed the safe filename validation in
## `parse_attrs()`.  It does not interpret DOT semantics.  After completing the
## action, it resets all current-figure globals so the state machine can resume
## ordinary Markdown processing.
##
## @local path Materialized asset path used by render or replacement.
## @local link Markdown image target used by graphical replacement.
##
## @par STDIN
## Nothing is read directly from STDIN.
## @par STDOUT
## Depending on `mode`, writes selected raw Markdown, a rendered asset path, a
## replacement fenced block, a graphical image reference, and optional caption.
## @par STDERR
## Invalid state is reported through `fail()`.
##
## @par Globals
## Reads `payload`, `mode`, `fmt`, `wantsrc`, `want`, `figdir`, `linkprefix`,
## `raw`, `id`, `alt`, and `caption`.  Resets `state`, `raw`, `meta`, `payload`,
## `id`, `fmt`, `alt`, `caption`, `fence_char`, and `fence_len` after success.
##
## @par Side Effects
## Render mode writes and closes a `.txt` or `.dot` asset beneath `figdir`.
## Replacement may read that asset through `emit_embedded()`.  Invalid or
## inconsistent mode/source state terminates processing through `fail()`.
##
...
function finish_figure(    path, link) {
  ...
  if (mode == "select") {
    ...
    path = figdir "/" id (fmt == "text" ? ".txt" : ".dot")
    ...
    if (want == "text" || want == "dot") {
      path = figdir "/" id (fmt == "text" ? ".txt" : ".dot")
      emit_embedded(path, want)
    } else {
      link = (linkprefix != "" ? linkprefix : figdir) "/" id "." want
      print "![" alt "](" link ")"
    }
