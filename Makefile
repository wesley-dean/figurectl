...
BUILD_ASSEMBLER := scripts/build-artifact.bash
CORE_BASH_SOURCE_FILES := lib/format-registry.bash lib/renderers/graphviz.bash
INPUT_PLUGIN_SOURCE_FILES := $(sort $(wildcard lib/plugins/input/*.bash))
OUTPUT_PLUGIN_SOURCE_FILES := $(sort $(wildcard lib/plugins/output/*.bash))
ENTRYPOINT_SOURCE := src/orchestrator.bash
ARTIFACT_BASH_SOURCE_FILES := $(CORE_BASH_SOURCE_FILES) $(INPUT_PLUGIN_SOURCE_FILES) $(OUTPUT_PLUGIN_SOURCE_FILES) $(ENTRYPOINT_SOURCE)
MAINTAINED_BASH_FILES := $(BUILD_ASSEMBLER) $(ARTIFACT_BASH_SOURCE_FILES)
...
PROCESSOR_AWK_FILES := \
	...
	lib/awk/actions.awk \
	lib/awk/main.awk
DOT_STYLE_AWK_FILE := lib/awk/dot-style.awk
MAINTAINED_AWK_FILES := $(PROCESSOR_AWK_FILES) $(DOT_STYLE_AWK_FILE)
...
VERSION ?= 0.0.0-dev
BUILD_COMMIT ?= $(shell commit="$$(git rev-parse --short=12 HEAD 2>/dev/null || printf 'unknown')"; if git rev-parse --is-inside-work-tree >/dev/null 2>&1 && [[ -n "$$(git status --porcelain --untracked-files=normal -- Makefile dependencies.txt src lib scripts 2>/dev/null)" ]]; then printf '%s-dirty' "$$commit"; else printf '%s' "$$commit"; fi)
...
$(DIST_DEV_SCRIPT): FORCE $(BUILD_ASSEMBLER) $(ARTIFACT_BASH_SOURCE_FILES) $(MAINTAINED_AWK_FILES)
	mkdir -p "$(DIST_DIR)"
	bash "$(BUILD_ASSEMBLER)" \
		"$@.tmp" \
		"$(PROJECT_NAME)" \
		"$(VERSION)" \
		"$(BUILD_DATE)" \
		"$(BUILD_COMMIT)" \
		"$(CORE_BASH_SOURCE_FILES)" \
		"$(INPUT_PLUGIN_SOURCE_FILES)" \
		"$(OUTPUT_PLUGIN_SOURCE_FILES)" \
		"$(PROCESSOR_AWK_FILES)" \
		"$(DOT_STYLE_AWK_FILE)" \
		"$(ENTRYPOINT_SOURCE)"
	chmod 0755 "$@.tmp"
...
## Embedded AWK documentation is intentionally removed by the same transform; the
## complete behavior suite proves that those removed lines are comments rather than
## executable AWK data.
...
## Validate maintained Bash and portable-AWK source.
check:
	@for source in $(MAINTAINED_BASH_FILES); do bash -n "$$source"; done
	shellcheck $(MAINTAINED_BASH_FILES)
	@awk \
		-v source_spec='text=txt;dot=dot' \
		-v mode=select \
		-v wantsrc=text \
		-v wantkind=fence \
		-v wantext=txt \
		-v wantinfo=text \
		-f lib/awk/common.awk \
		-f lib/awk/capabilities.awk \
		-f lib/awk/metadata.awk \
		-f lib/awk/fences.awk \
		-f lib/awk/actions.awk \
		-f lib/awk/main.awk \
		/dev/null >/dev/null
	@printf 'digraph {\n}\n' | awk -v stylefile=/dev/null -f "$(DOT_STYLE_AWK_FILE)" >/dev/null

## Format maintained Bash using the same arguments configured for MegaLinter.
format:
	shfmt $(SHFMT_ARGUMENTS) -w $(MAINTAINED_BASH_FILES)

## Run the public behavior suite against every generated artifact flavor.
test: build
	@set -e; for artifact in $(DIST_SCRIPTS); do printf 'Testing %s\n' "$$artifact"; FIGURECTL_ARTIFACT="$${PWD}/$$artifact" bats $(TEST_SCRIPTS); done

## Run every artifact flavor and emit one JUnit report per flavor.
test-report: build
	@mkdir -p "$(TEST_RESULTS_DIR)"
	@set -e; for entry in 'development|$(DIST_DEV_SCRIPT)' 'stripped|$(DIST_SCRIPT)' 'minified|$(DIST_MIN_SCRIPT)'; do label="$${entry%%|*}"; artifact="$${entry#*|}"; FIGURECTL_ARTIFACT="$${PWD}/$$artifact" bats --formatter junit $(TEST_SCRIPTS) >"$(TEST_RESULTS_DIR)/bats-$${label}.xml"; done
